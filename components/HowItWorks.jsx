...
export default function HowItWorks() {
  ...
  return (
    <section ref={sectionRef} className="relative bg-[#050507] text-white pt-32 pb-48" style={{ overflowX: "clip" }}>
      ...
      <div className="absolute top-0 right-0 pointer-events-none"
        style={{
          ...
          background: "radial-gradient(circle, rgba(180,16,29,0.06) 0%, transparent 60%)",
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="text-center md:text-left mb-16 md:mb-32">
          ...
          <h2 className="font-bebas text-white m-0 text-[clamp(48px,8vw,96px)] leading-[0.95] tracking-[0.02em]">
            From Plug-In to <span className="text-[#b4101d] drop-shadow-[0_0_30px_rgba(180,16,29,0.5)]">Profit</span>
          ...
          <p className="mt-6 text-base leading-[1.75] max-w-[500px] text-white/50 mx-auto md:mx-0">
            Auto Forge connects your DMS, CRM, inventory feeds, and customer touchpoints into a single intelligent ecosystem.
          </p>
        </div>
        ...
        {isMobile ? (
              ...
              <div key={i} className="flex flex-col relative">
                ...
                <div className="relative w-full aspect-[4/3] rounded-3xl border border-white/10 bg-white/[0.02] overflow-visible shadow-2xl shadow-black mb-16 mt-6">
                   <div className="absolute inset-0 rounded-3xl overflow-hidden bg-[#050507]">
                     <video src={step.video} autoPlay loop muted playsInline className="w-full h-full object-cover opacity-80" />
                     <div className="absolute inset-0 bg-gradient-to-t from-[#050507] via-transparent to-transparent opacity-90" />
                   </div>
                   <img src={step.carImg} alt={step.label} className="absolute bottom-[-15%] right-[-10%] w-[120%] max-w-none" style={{ filter: "drop-shadow(0 20px 40px rgba(0,0,0,0.8))" }} />
                </div>
                {/* Text */}
                <div className="relative pl-8 border-l border-white/10">
                   <div className="absolute left-[-5px] top-0 w-2.5 h-2.5 rounded-full bg-[#b4101d] shadow-[0_0_12px_#b4101d]" />
                   <div className="text-[11px] uppercase tracking-[0.3em] font-medium text-[#b4101d] mb-4">Step {step.num}</div>
                   <h3 className="font-bebas text-5xl mb-2 text-white">{step.label}</h3>
          ...
          <div ref={containerRef} className="flex items-start gap-16 lg:gap-24 relative">
            ...
            <div className="w-[45%] lg:w-[40%] relative pb-[30vh]">
              ...
              {STEPS.map((step, i) => {
                ...
                return (
                  <div key={i} className={`step-item relative pl-24 py-32 transition-all duration-700 ${isActive ? "opacity-100" : "opacity-30 blur-[2px]"}`}>
                    
                    {/* Glowing Dot */}
                    <div className="absolute left-[31px] top-[140px] w-2.5 h-2.5 rounded-full z-10 transition-all duration-500" 
                      style={{ 
                        background: isActive ? "#b4101d" : "#333",
                        boxShadow: isActive ? "0 0 20px 4px rgba(180,16,29,0.8)" : "none",
                        transform: isActive ? "scale(1.5)" : "scale(1)"
                      }} 
                    />

                    {/* Step Number */}
                    <div className="absolute left-0 top-32 font-bebas text-4xl transition-colors duration-500"
                      style={{ color: isActive ? "#b4101d" : "rgba(255,255,255,0.2)" }}
                    >
                      {step.num}
                    </div>

                    <h3 className="font-bebas text-[3.5rem] lg:text-[4.5rem] mb-4 text-white leading-none tracking-[0.02em]">{step.label}</h3>
                    <h4 className="text-[1.3rem] mb-4 text-white/90 font-medium">{step.title}</h4>
                    <p className="text-base leading-[1.8] text-white/60">{step.desc}</p>
                  </div>
                );
              })}
            </div>
            ...
            <div className="w-[55%] lg:w-[60%] sticky top-[15vh] h-[70vh] flex items-center justify-center">
              ...
              <div className="relative w-full max-w-[800px] aspect-[4/3] rounded-[32px] border border-white/10 bg-white/[0.02] backdrop-blur-xl shadow-[0_30px_100px_rgba(0,0,0,0.8)] overflow-visible group">
                ...
                <div className="absolute top-6 left-6 flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/40 border border-white/10 backdrop-blur-md z-20">
                   ...
                   <span className="text-[9px] uppercase tracking-widest text-white/70">System Active</span>
                </div>
                
                {/* Top right decorative corners */}
                <svg className="absolute top-0 right-0 w-16 h-16 text-white/20 z-20 pointer-events-none" viewBox="0 0 64 64" fill="none">
                  <path d="M64 0H0" stroke="currentColor" strokeWidth="2" strokeDasharray="4 4" />
                  <path d="M64 0V64" stroke="currentColor" strokeWidth="2" strokeDasharray="4 4" />
                </svg>
                <svg className="absolute bottom-0 left-0 w-16 h-16 text-white/20 z-20 pointer-events-none" viewBox="0 0 64 64" fill="none">
                  <path d="M0 64H64" stroke="currentColor" strokeWidth="2" strokeDasharray="4 4" />
                  <path d="M0 64V0" stroke="currentColor" strokeWidth="2" strokeDasharray="4 4" />
                </svg>
                  ...
                  return (
                    <img 
                      ...
                      style={{ 
                        ...
                        transform: isActive ? "translate(0, 0) scale(1)" : "translate(40px, 40px) scale(0.95)",
                        filter: isActive ? "drop-shadow(0 40px 60px rgba(0,0,0,0.8)) brightness(1.1)" : "drop-shadow(0 10px 20px rgba(0,0,0,0.5)) brightness(0.5)",
